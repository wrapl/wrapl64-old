...
typedef struct section_class_t {
...
static uint32_t NoOfSections = 0;
static stringmap_t Libraries[1] = {STRINGMAP_INIT};
static stringmap_t Requires[1] = {STRINGMAP_INIT};
static stringmap_t Globals[1] = {STRINGMAP_INIT};
static stringmap_t WeakGlobals[1] = {STRINGMAP_INIT};
static stringmap_t Symbols[1] = {STRINGMAP_INIT};
static stringmap_t Exports[1] = {STRINGMAP_INIT};
static stringmap_t Defines[1] = {STRINGMAP_INIT};
static stringmap_t Dependencies[1] = {STRINGMAP_INIT};
...
static void export_new(const char *Internal, const char *External, uint32_t Flags) {
	if (!stringmap_search(Exports, External)) {
		export_t *Export = new(export_t);
		Export->Internal = Internal;
		Export->Flags = Flags;
		Export->Section = 0;
		stringmap_insert(Exports, External, Export);
	}
}
...
static symbol_t *symbol_new(const char *Name, section_t *Section, uint32_t Offset) {
...
static library_section_t *library_section_new(const char *Path) {
	static section_class_t Class = {
	...
	Section->Index = SEC_UNUSED;
	Section->Class = &Class;
	Section->Path = Path;
	stringmap_insert(Libraries, Path, Section);
	return Section;
};
...
static void import_section_debug(import_section_t *Section, FILE *File) {
	fprintf(File, "%d: import section: %d.%s\n", Section->Index, ((section_t *)Section->Library)->Index, Section->Name);
};
...
static import_section_t *import_section_new(library_section_t *Library, const char *Name, int Type) {
	static section_class_t Class = {
...
static bss_section_t *bss_section_new(uint32_t Size) {
	static section_class_t Class = {
		default_section_setup,
		default_section_relocate,
		default_section_export,
...
static void tbss_section_relocate(tbss_section_t *Section, relocation_t *Relocation, uint32_t *Target) {
	...
	printf("%s: testing tbss relocations @ %d[%x, %d]:\n\t", Section->Name, Relocation->Position, Relocation->Flags, Relocation->Size);
	for (int I = 0; I < 12; ++I) printf("%x ", Temp[I]);
	printf("\n");
	Relocation->Section = Section;
};
...
static tbss_section_t *tbss_section_new(uint32_t Size) {
	static section_class_t Class = {
		default_section_setup,
...
static symbol_section_t *symbol_section_new(const char *Name) {
	static section_class_t Class = {
		default_section_setup,
...
static void symbols_section_relocate(symbols_section_t *Section, relocation_t *Relocation, uint32_t *Target) {
	...
	if (Section->Symbols) {
		...
		Symbol = (section_t *)stringmap_search(Symbols, Name);
		if (!Symbol) {
			Name = strdup(Name);
			Symbol = (section_t *)symbol_section_new(Name);
			stringmap_insert(Symbols, Name, Symbol);
		};
	} else {
		Symbol = Section->ASymbols[*Target];
		if (!Symbol) {
			Section->ASymbols[*Target] = Symbol = (section_t *)symbol_section_new(0);
		};
	};
	*Target = 0;
...
static void symbols_section_export(symbols_section_t *Section, uint32_t Offset, export_t *Export) {
	...
	if (Section->Symbols) {
		...
		if (!Symbol) {
			Section->ASymbols[Offset] = Symbol = (section_t *)symbol_section_new(0);
		};
	};
...
static symbols_section_t *symbols_section_new(asection *Sect, bfd *Bfd, int Anon) {
	static section_class_t Class = {
	...
	if (strcmp(Sect->name, ".ctors") == 0) {
		...
		for (int I = Section->NoOfRelocs - 1; I >= 0; --I, ++J) {
			...
			if (!strcmp(Sym->name, "_GLOBAL_OFFSET_TABLE_")) {
				...
				symbol_t *Symbol;
				do {
					Symbol = (symbol_t *)stringmap_search(BfdInfo->Locals, Sym->name);
					if (Symbol) break;
					Symbol = (symbol_t *)stringmap_search(Globals, Sym->name);
					if (Symbol) break;
					Symbol = (symbol_t *)stringmap_search(WeakGlobals, Sym->name);
					if (Symbol) break;
#ifdef WINDOWS
					...
					Symbol = symbol_new(Sym->name, import_section_new(UnknownSymbols, Sym->name, 0), 0);
				} while (0);
				section_t *Section2 = Symbol->Section;
				if (Section2 == 0) {
					printf("%s: unresolved symbol %s.\n", Bfd->filename, Sym->name);
					if (StopOnUnknown) exit(1);
					Section2 = import_section_new(UnknownSymbols, Sym->name, 0);
				};
...
static void bfd_section_debug(bfd_section_t *Section, FILE *File) {
	fprintf(File, "%d: text section: %d[%s,%s] %s:%s[%x]", Section->Index, Section->Size, (Section->Flags & FLAG_GC) ? "GC" : "NOGC", (Section->Flags & FLAG_DELAY) ? "DELAY" : "NODELAY", Section->BfdInfo->FileName, Section->Name, Section->Sect->flags);
...
static bfd_section_t *bfd_section_new(asection *Sect, bfd *Bfd, bfd_info_t *BfdInfo) {
	static section_class_t Class = {
...
static void tdata_section_debug(bfd_section_t *Section, FILE *File) {
	fprintf(File, "%d: tdata section: %d[%s,%s] %s:%s[%x]", Section->Index, Section->Size, (Section->Flags & FLAG_GC) ? "GC" : "NOGC", (Section->Flags & FLAG_DELAY) ? "DELAY" : "NODELAY", Section->BfdInfo->FileName, Section->Name, Section->Sect->flags);
...
static tdata_section_t *tdata_section_new(asection *Sect, bfd *Bfd, bfd_info_t *BfdInfo) {
	static section_class_t Class = {
		(void (*)(section_t *))bfd_section_setup,
		default_section_relocate,
		default_section_export,
...
static got_section_t *got_section_new() {
	static section_class_t Class = {
...
static methods_section_t *method_section_new() {
	static section_class_t Class = {
...
static typedfn_section_t *typedfn_section_new() {
	static section_class_t Class = {
...
static initial_section_t *initial_section_new() {
	static section_class_t Class = {
...
static void constant_section_debug(constant_section_t *Section, FILE *File) {
	fprintf(File, "%d: constant section: %d[%d]\n", Section->Index, Section->Init->Index, Section->Offset);
};
...
static constant_section_t *constant_section_new(void) {
	static section_class_t Class = {
...
static void constants_section_relocate(constants_section_t *Section, relocation_t *Relocation, uint32_t *Target) {
	if (Section->Sect) {
		...
		for (int I = 0; I < Count; I++) Section->Constants[I] = constant_section_new();
		uint8_t *Code;
...
static constants_section_t *constants_section_new(asection *Sect, bfd *Bfd, bfd_info_t *BfdInfo) {
	static section_class_t Class = {
		invalid_section_setup,
...
static void add_bfd_section(bfd *Bfd, asection *Sect, bfd_info_t *BfdInfo) {
	if (strcmp(Sect->name, ".symbols") == 0) {
		if (Sect->size) Sect->userdata = symbols_section_new(Sect, Bfd, 0);
	} else if (strcmp(Sect->name, ".asymbol") == 0) {
		Sect->userdata = symbols_section_new(Sect, Bfd, 1);
	} else if (strcmp(Sect->name, ".methods") == 0) {
		if (Sect->size) {
			methods_section_t *Section = method_section_new();
			bfd_section_t *Block = bfd_section_new(Sect, Bfd, BfdInfo);
		...
		if (Sect->size) {
			typedfn_section_t *Section = typedfn_section_new();
			bfd_section_t *Block = bfd_section_new(Sect, Bfd, BfdInfo);
			Block->Next = Section->Blocks;
			Section->Blocks = Block;
			Sect->userdata = Block;
		...
		if (Sect->size) {
			initial_section_t *Section = initial_section_new();
			bfd_section_t *Block = bfd_section_new(Sect, Bfd, BfdInfo);
			Block->Next = Section->Blocks;
			Section->Blocks = Block;
			Sect->userdata = Block;
		...
		if (Sect->size) Sect->userdata = constants_section_new(Sect, Bfd, BfdInfo);
	} else if (Sect->flags & SEC_LOAD) {
		if (Sect->flags & SEC_THREAD_LOCAL) {
			//printf("%s: .tdata support is experimental!\n", Bfd->filename);
			Sect->userdata = tdata_section_new(Sect, Bfd, BfdInfo);
		} else {
			Sect->userdata = bfd_section_new(Sect, Bfd, BfdInfo);
		};
	} else if (Sect->flags & SEC_ALLOC) {
		if (Sect->flags & SEC_THREAD_LOCAL) {
			//printf("%s: .tbss support is experimental!\n", Bfd->filename);
			Sect->userdata = tbss_section_new(bfd_get_section_size(Sect));
		} else {
			Sect->userdata = bss_section_new(bfd_get_section_size(Sect));
		};
...
static void add_bfd(bfd *Bfd, int AutoExport) {
	if (bfd_check_format(Bfd, bfd_object)) {
		...
		memset(BfdInfo->Locals, 0, sizeof(BfdInfo->Locals));
		...
		for (int I = NoOfSymbols - 1; I >= 0; --I) {
			...
			if (Sym->flags & BSF_GLOBAL) {
				...
				symbol_t *Symbol = symbol_new(Name, (section_t *)Sym->section->userdata, Sym->value);
				if (Sym->section->userdata && Sym->value == 0) {
					((section_t *)Sym->section->userdata)->Name = Name;
				};
				stringmap_insert(BfdInfo->Locals, Name, Symbol);
				stringmap_insert(Globals, Name, Symbol);
#ifdef WINDOWS
				if (AutoExport) export_new(Name, Name + 1, 0);
#else
				if (AutoExport) export_new(Name, Name, 0);
#endif
			} else if (Sym->section == bfd_com_section_ptr) {
				symbol_t *Symbol = (symbol_t *)stringmap_search(Globals, Sym->name);
				bss_section_t *Section;
				if (!Symbol) {
					const char *Name = strdup(Sym->name);
					Section = bss_section_new(0);
					Symbol = symbol_new(Name, (section_t *)Section, 0);
					stringmap_insert(Globals, Name, Symbol);
					stringmap_insert(BfdInfo->Locals, Name, Symbol);
				} else {
				...
				symbol_t *Symbol = symbol_new(Name, (section_t *)Sym->section->userdata, Sym->value);
				if (Sym->section->userdata && Sym->value == 0) ((section_t *)Sym->section->userdata)->Name = Name;
				stringmap_insert(WeakGlobals, Name, Symbol);
/*
#ifdef WINDOWS
				if (AutoExport) export_new(Name, Name + 1, 0);
#else
...
static int module_section_export_setup_fn(const char *Name, export_t *Export, void *Data) {
	if (Export->Section == 0) {
		symbol_t *Symbol = (symbol_t *)stringmap_search(Globals, Export->Internal);
		if (Symbol == 0) Symbol = (symbol_t *)stringmap_search(WeakGlobals, Export->Internal);
		if (Symbol) {
...
static void module_section_setup(module_section_t *Section) {
	stringmap_foreach(Section->Exports, 0, module_section_export_setup_fn);
};

static int export_debug_fn(const char *Name, export_t *Export, FILE *File) {
	fprintf(File, "\texport: %s -> %d[%d]\n", Name, Export->Section->Index, Export->Offset);
	return 0;
}

static int export_write_fn(const char *Name, export_t *Export, gzFile *File) {
	if (Export->Section) {
		uint32_t Temp = Export->Flags; gzwrite(File, &Temp, 1);
		Temp = Export->Section->Index; gzwrite(File, &Temp, 4);
		Temp = Export->Offset; gzwrite(File, &Temp, 4);
		Temp = strlen(Name) + 1; gzwrite(File, &Temp, 4);
		gzwrite(File, Name, Temp);
	}
	return 0;
}
...
static void module_section_debug(module_section_t *Section, FILE *File) {
	...
	stringmap_foreach(Section->Exports, File, export_debug_fn);
};

static int module_section_export_size_fn(const char *Name, export_t *Export, uint32_t *Size) {
	Size[0] += 1 + 4 + 4 + 4 + strlen(Name) + 1;
	return 0;
}

static uint32_t module_section_size(module_section_t *Section) {
	uint32_t Size = 1 + 1 + 4;
	stringmap_foreach(Section->Exports, &Size, module_section_export_size_fn);
	return Size;
};
...
static void module_section_write(module_section_t *Section, gzFile File) {
	...
	Temp = 0; gzwrite(File, &Temp, 1);
	Temp = Section->Exports->Size; gzwrite(File, &Temp, 4);
	stringmap_foreach(Section->Exports, File, export_write_fn);
...
static module_section_t *module_section_new(const char *Name) {
	static section_class_t Class = {
...
static void add_define(const char *Name, const char *Value) {
	stringmap_insert(Defines, Name, Value);
};
...
static int script_file_module(lua_State *State) {
	...
	library_section_t *LibrarySection = (library_section_t *)stringmap_search(Libraries, Path);
	if (LibrarySection == 0) {
        LibrarySection = library_section_new(Path);
        stringmap_insert(Globals, Name, symbol_new(Name, (section_t *)LibrarySection, 0));
	};
...
static int script_file_import(lua_State *State) {
    ...
    import_section_t *ImportSection = (import_section_t *)stringmap_search(Library->Section->Imports, Internal);
    if (ImportSection == 0) {
        ImportSection = import_section_new(Library->Section, Internal, Flags);
    };
    symbol_t *Symbol = symbol_new(External, (section_t *)ImportSection, 0);
    stringmap_insert(Globals, External, Symbol);
    return 0;
};
...
static int script_file_export(lua_State *State) {
    ...
    export_new(Internal, External, Flags);
    return 0;
};

static int script_file_require(lua_State *State) {
	char *Path = lua_tostring(State, 1);
	library_section_t *Section = stringmap_search(Libraries, Path);
	if (!Section) {
		Section = library_section_new(Path);
		Section->Required = 1;
		stringmap_insert(Libraries, Path, Section);
	}
	stringmap_insert(Requires, Path, Section);
	return 0;
};
...
static int script_file_submodule(lua_State *State) {
    ...
    if (External) export_new(Internal, External, 0);
    stringmap_insert(Globals, Internal, symbol_new(Internal, (section_t *)module_section_new(External), 0));
    return 0;
};
...
static int script_file_subexport(lua_State *State) {
    ...
    module_section_t *SubModule = ((symbol_t *)stringmap_search(Globals, SubName))->Section;
    if (SubModule == 0) {
	...
	Export->Flags = Flags;
	Export->Section = 0;
	stringmap_insert(SubModule->Exports, External, Export);
    return 0;
};

static int lua_define_add_fn(const char *Name, const char *Value, lua_State *State) {
...
static void add_script_file(const char *FileName, int AutoExport) {
	...
	stringmap_foreach(Defines, State, lua_define_add_fn);
...
static int definition_file_symbol(lua_State *State) {
	...
	export_new(Internal, External, 0);
};
	...
	if (bfd_check_format(Bfd, bfd_object)) {
		...
		for (int I = NoOfSymbols - 1; I >= 0; --I) {
			...
			if (Sym->flags & BSF_GLOBAL) {
				...
				if (ImportSection == 0) {
					ImportSection = import_section_new(LibrarySection, Name, 0);
					symbol_t *Symbol = symbol_new(Name, (section_t *)ImportSection, 0);
					stringmap_insert(Globals, Name, Symbol);
#ifdef WINDOWS
					//char *_Name = malloc(strlen(Name) + 2);
					//_Name[0] = '_';
					//strcpy(_Name + 1, Name);
					//stringmap_insert(Globals, _Name, Symbol);
#endif
				};
			};
		};
...
#endif
#ifdef MACOSX
	Module[strlen(Module) - 6] = 0;
#endif
	//printf("Adding shared library %s -> %s\n", FileName, Module);
	
	library_section_t *LibrarySection = (library_section_t *)stringmap_search(Libraries, Module);
	if (LibrarySection == 0) {
		LibrarySection = library_section_new(Module);
		bfd *Bfd = bfd_openr(FileName, 0);
		if (Bfd == 0) {
			printf("%s: error reading file.\n", FileName);
			return;
...
static stringmap_t SupportedFiles[1] = {STRINGMAP_INIT};
...
static int dependency_print_fn(const char *Name, void *Value, void *Data) {
	printf("\t%s\n", Name);
	return 0;
};

static int require_write_fn(const char *Name, library_section_t *Library, gzFile *File) {
	if (Library->Required) {
		uint32_t Temp = Library->Flags; gzwrite(File, &Temp, 1);
		Temp = strlen(Library->Path) + 1; gzwrite(File, &Temp, 4);
		gzwrite(File, Library->Path, Temp);
	}
	return 0;
}

static int export_import_write_fn(const char *Name, export_t *Export, FILE *File) {
	if (Export->Section) fprintf(File, "import(\"%s\")\n", Name);
	return 0;
}

static int export_require_fn(const char *Name, export_t *Export, void *Data) {
	if (Export->Section == 0) {
		symbol_t *Symbol = (symbol_t *)stringmap_search(Globals, Export->Internal);
		if (Symbol == 0) Symbol = (symbol_t *)stringmap_search(WeakGlobals, Export->Internal);
		if (Symbol) {
...
static int export_list_fn(const char *Name, export_t *Export, FILE *File) {
	if (Export->Section) fprintf(File, "export: %s -> %d[%d]\n", Name, Export->Section->Index, Export->Offset);
	return 0;
}

static int require_list_fn(const char *Name, library_section_t *Require, FILE *File) {
	fprintf(File, "require: %s[%d]\n", Require->Path, Require->Flags);
	return 0;
}
...
int main(int Argc, char **Argv) {
	...
	if (Argc < 2) {
		...
		bfd_init();
		//stringmap_insert(Globals, "_GLOBAL_OFFSET_TABLE_", symbol_new("_GLOBAL_OFFSET_TABLE_", got_section_new(), 0));
		UnknownSymbols = library_section_new("__unknown__");
		...
		if (DependencyMode) {
			...
			stringmap_foreach(Dependencies, 0, dependency_print_fn);
			return 0;
		};
		stringmap_foreach(Exports, 0, export_require_fn);
		if (ExportFile) {
			...
			stringmap_foreach(Exports, File, export_import_write_fn);
			fclose(File);
		};
		...
		if (ListFile) {
			...
			stringmap_foreach(Exports, File, export_list_fn);
			stringmap_foreach(Requires, File, require_list_fn);
			fclose(File);
		};
		if (OutFile) {
			...
			Temp = Exports->Size; gzwrite(File, &Temp, 4);
			Temp = Requires->Size; gzwrite(File, &Temp, 4);
			for (section_t *Section = Sections.Head; Section; Section = Section->Next) section_write(Section, File);
			stringmap_foreach(Exports, File, export_write_fn);
			stringmap_foreach(Libraries, File, require_write_fn);
			gzclose(File);
		};
	};
};
